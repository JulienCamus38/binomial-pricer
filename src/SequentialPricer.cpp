...
double SequentialPricer::computePrice(Option& option)
{
    ...
    double h = option.T / option.N;
    double u = exp(option.sigma * sqrt(h));
    double d = 1.0 / u;
    double r = exp(option.R * h);
    double p = (r - d) / (u - d);
    ...
    for (int i = 0; i <= option.N; ++i)
    {
        ...
        leaves[i] = (option.isCall) ? max(ST - option.K, 0.0) : max(option.K - ST, 0.0);
    ...
    for (int i = option.N - 1; i >= 0; --i)
    {
        for (int j = 0; j <= i; ++j)
        {
            ...
            if (option.isAmerican)
            {
                ...
                leaves[j] = max(leaves[j], (option.isCall) ? max(ST - option.K, 0.0) : max(option.K - ST, 0.0));
            }
